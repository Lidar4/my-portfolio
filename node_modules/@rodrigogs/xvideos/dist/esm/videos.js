...
const uniqueSortedPages = (pages, currentPage) => {
    return Array.from(new Set([...pages, currentPage])).sort((left, right) => left - right);
};
const parsePages = ($) => {
...
const parseVideo = ($, element) => {
    ...
    const profileLink = $video
        ...
        .first();
    const profileName = normalizeText(profileLink.find('.name').first().text()) ||
        normalizeText(profileLink.text());
    const duration = normalizeText($video
        .find('.video-metadata .duration, .duration-container .duration, p.metadata .duration, p.title .duration')
        .first()
        .text());
    const thumbnailCandidate = normalizeText(link.find('img').first().attr('data-src') ||
...
const normalizeDetailsManyOptions = ({ concurrency = 4, retries = 0, retryDelayMs = 0, minDelayMs = 0, } = {}) => {
...
const createStartGate = (minDelayMs) => {
    if (minDelayMs <= 0) {
        return async () => { };
    }
...
const retryDetails = async (input, retries, retryDelayMs) => {
    let attempt = 0;
    while (true) {
        try {
...
const buildListResult = (page, html, loadPage) => {
    const $ = load(html);
...
const buildEmptyListing = (page, loadPage) => ({
    videos: [],
...
const loadListingPage = async (page, candidates, loadPage) => {
    let lastError;
...
const createIndexQueryCandidates = (page) => {
    if (page === 1) {
        return ['/'];
    }
...
const dashboard = async ({ page = 1, } = {}) => {
    assertPage(page);
    return loadListingPage(page, createIndexQueryCandidates(page), (targetPage) => dashboard({ page: targetPage }));
};
const fresh = async ({ page = 1, } = {}) => {
    assertPage(page);
    const candidates = page === 1 ? ['/new', '/new/1'] : [`/new/${page}`];
    return loadListingPage(page, candidates, (targetPage) => fresh({ page: targetPage }));
};
const verified = async ({ page = 1, } = {}) => {
    ...
    return loadListingPage(page, candidates, (targetPage) => verified({ page: targetPage }));
};
const best = async ({ year, month, page = 1, } = {}) => {
    ...
    return loadListingPage(page, candidates, (targetPage) => best({ year: bestYear, month: bestMonth, page: targetPage }));
};
const assertCategory = (categorySlug) => {
    if (!/^[a-zA-Z0-9_-]+$/.test(categorySlug)) {
        throw new Error(`Invalid category: ${categorySlug}`);
    }
};
const createCategoryCandidates = (categorySlug, page) => {
    if (page === 1) {
...
const category = async ({ category: categorySlug, page = 1, }) => {
    assertPage(page);
    assertCategory(categorySlug);
    try {
        return await loadListingPage(page, createCategoryCandidates(categorySlug, page), (targetPage) => category({
        ...
        const statusCode = error
            .response?.statusCode;
        if (statusCode === 404) {
            return buildEmptyListing(page, (targetPage) => category({ category: categorySlug, page: targetPage }));
        }
        throw error;
    }
};
const createSearchCandidate = (page, pageValue, { k = '', sort = 'relevance', durf = 'allduration', datef = 'all', quality = 'all', }) => {
...
const search = async ({ page = 1, k = '', sort = 'relevance', durf = 'allduration', datef = 'all', quality = 'all', } = {}) => {
    assertPage(page);
    ...
    return loadListingPage(page, candidates, (targetPage) => search({ ...options, page: targetPage }));
};
...
const parseEngagement = ($, html) => {
    const voteText = normalizeText($('[class*="rating"], [class*="vote"]').first().text()) ||
        html;
    const voteMatch = voteText.match(/([0-9]+(?:[.,][0-9]+)?\s*[KMB]?)\s*votes?/i);
...
const parseWatchCount = (jsonLd, views) => {
    ...
    for (const candidate of candidates) {
        if (candidate && typeof candidate === 'object') {
            const value = candidate.userInteractionCount;
            if (typeof value === 'number' && Number.isFinite(value)) {
                return value;
            }
            const parsed = parseNumberWithSuffix(typeof value === 'string' ? value : '');
            if (parsed > 0) {
                return parsed;
            }
        }
...
const readDetailViews = ($, html) => {
    const directViews = normalizeText($('#v-views strong.mobile-hide').first().text()) ||
    ...
    const match = html.match(/id="v-views"[\s\S]*?<strong[^>]*>([^<]+)<\/strong>/i);
...
const parseResolutions = (manifest) => {
    return Array.from(manifest.matchAll(/RESOLUTION=(\d+)x(\d+)/gi), (match) => ({
        width: Number.parseInt(match[1], 10),
        height: Number.parseInt(match[2], 10),
    })).filter((resolution) => Number.isFinite(resolution.width) && Number.isFinite(resolution.height));
};
const resolveVideoSize = async (files, width, height) => {
    if (width && height) {
        return { width, height };
    }
    if (!files.HLS) {
...
const details = async ({ url }) => {
    ...
    const size = await resolveVideoSize(files, readMeta($, 'og:video:width') || sizeMatch?.[1] || '', readMeta($, 'og:video:height') || sizeMatch?.[2] || '');
    return {
        ...
        thumbnailUrls: parseStringArray(jsonLd.thumbnailUrl).length > 0
...
const detailsMany = async (inputs, options = {}) => {
    ...
    const { concurrency, retries, retryDelayMs, minDelayMs } = normalizeDetailsManyOptions(options);
    const reserveStart = createStartGate(minDelayMs);
    const items = new Array(normalizedInputs.length);
    let cursor = 0;
    const worker = async () => {
        while (true) {
            const index = cursor;
    ...
    await Promise.all(Array.from({ length: Math.min(concurrency, normalizedInputs.length) }, async () => worker()));
